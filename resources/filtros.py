def normalize_path_params(cidade=None, estrelas_min=0, estrelas_max=5, diaria_min=0, diaria_max=10000,
...
consulta_sem_cidade = "SELECT * FROM hoteis WHERE (estrelas >= ? and estrelas <= ?) and " \
                      "(diaria >= ? and diaria <= ?) LIMIT ? OFFSET ?"

consulta_com_cidade = "SELECT * FROM hoteis WHERE (estrelas >= ? and estrelas <= ?) and " \
